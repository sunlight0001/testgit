package git.study;

public class MyFirstCommit {
	
	private void syso() {
		// TODO Auto-generated method stub

	}
	
	public static void main(String[] args) {
		
		System.out.println("this is add in dev branch");
	}

	public void sys(){
		System.out.println("master and dev");
		System.out.println("a new commit for syn");
		System.out.println("a new commit for yuancheng gg");
		
 		System.out.println("a new commit for syn  cc and ee aa");
		System.out.println("a new commit for syn  bb and xx and yy  and cc");
		
		System.out.println("this is edit in github all good is good tool");
		System.out.println("this is edit in github  at two and one and two");
	}
}
